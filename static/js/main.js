(function () {
    ...
    function initTypewriters() {
        ...
        targets.forEach((host) => {
            const text = host.getAttribute('data-typed-text') || '';
            const target = host.querySelector('.typed-target');
            if (!target) return;

            // Reduced-motion / empty-text path: skip the type animation and
            // hand straight off to the static glass-sheen layer the CSS draws
            // via .typed-line::before once .is-typed-complete is set.
            if (prefersReducedMotion || !text) {
                target.textContent = text;
                host.classList.add('is-typed-complete');
                return;
            }

            // SEO fallback: the .typed-target may be pre-filled server-side
            // with the full headline so crawlers and no-JS users still see it.
            // Clear it now that JS is taking over so the type animation runs
            // from an empty state.
            target.textContent = '';

            // Timing knobs (overridable per-element via data-* attrs):
            //   speed      — ms per char when typing
            //   startDelay — ms before the very first character
            // The old erase / hold / loop knobs are no longer used: the
            // headline now types in once and stays — CSS takes it from there.
            const speed      = Math.max(10, parseInt(host.getAttribute('data-typed-speed')       || '55',  10));
            const startDelay = Math.max(0,  parseInt(host.getAttribute('data-typed-start-delay') || '200', 10));
            ...
            function tick() {
                ...
                if (index > text.length) {
                    target.textContent = text;
                    // Typing finished. Add a marker class so the CSS can
                    // swap from the JS-driven .typed-target to the static
                    // ::before layer (full text + glass-sheen animation).
                    host.classList.add('is-typed-complete');
                    return;
                }
            ...
            // Kick off once the headline scrolls into view (so the animation
            // isn't already finished by the time the user gets there).
            if ('IntersectionObserver' in window) {
                const observer = new IntersectionObserver((entries) => {
                    entries.forEach((entry) => {
                        if (entry.isIntersecting) {
    ...
    function initMarquees() {
        ...
        document.querySelectorAll('[data-marquee]').forEach((marquee) => {
            ...
            const cards = track.querySelectorAll('[data-marquee-card]');
            if (cards.length === 0) return;

            // The track holds two copies of the source list (rendered server-side
            // for a seamless visual loop). We work in "logical position" units
            // measured in pixels of horizontal offset.
            ...
            // Re-measure on load + every resize. halfWidth is the distance from
            // position 0 back to the start of the duplicate half — the seam point.
            function measure() {
                if (cards.length < halfCount + 1) return;
                const firstDup = cards[halfCount];
                const trackRect = track.getBoundingClientRect();
                const dupRect = firstDup.getBoundingClientRect();
            ...
            function tick() {
                if (!isPaused && halfWidth > 0) {
                    ...
                    // When we've travelled the width of one full copy, reset
                    // to 0 — the duplicate is now in the original's spot, so
                    // the visual is identical and the loop is seamless.
            ...
            const counterCurrent = marquee.parentElement
                ? marquee.parentElement.querySelector('[data-marquee-current]')
                : null;
            // Try a broader scope for the counter — controls may sit outside the marquee element.
            const widerScope = marquee.closest('section') || document;
            const counterCurrentEl = counterCurrent || widerScope.querySelector('[data-marquee-current]');
            const counterTotalEl = widerScope.querySelector('[data-marquee-total]');

            function updateCounter() {
                if (!counterCurrentEl || halfWidth <= 0) return;
                const cardWidth = halfWidth / halfCount;
                if (cardWidth <= 0) return;
                const idx = (Math.round(position / cardWidth) % halfCount + halfCount) % halfCount;
                counterCurrentEl.textContent = (idx + 1).toString();
            }
            ...
            function step(direction) {
                ...
                applyTransform();
                updateCounter();
                window.setTimeout(() => track.classList.remove('is-stepping'), 520);
                pauseTemporarily(4000);
            }

            // Hover and keyboard focus pause the auto-scroll.
            marquee.addEventListener('mouseenter', () => { isPaused = true; });
            marquee.addEventListener('mouseleave', () => {
                if (!resumeTimer) isPaused = prefersReducedMotion;
            });
            ...
            // Wire prev/next buttons. They may live anywhere in the section,
            // so we search the surrounding container.
            const controlScope = marquee.closest('section') || document;
            const prevBtn = controlScope.querySelector('[data-marquee-prev]');
            const nextBtn = controlScope.querySelector('[data-marquee-next]');
            if (prevBtn) prevBtn.addEventListener('click', () => step(-1));
            if (nextBtn) nextBtn.addEventListener('click', () => step(1));

            // Update counter periodically so it reflects rAF-driven position.
            window.setInterval(updateCounter, 400);

            // Initial measure once images / fonts have settled.
            measure();
